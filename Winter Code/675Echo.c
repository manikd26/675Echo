#pragma config(UART_Usage, UART1, uartVEXLCD, baudRate19200, IOPins, None, None)
#pragma config(UART_Usage, UART2, uartNotUsed, baudRate4800, IOPins, None, None)
#pragma config(I2C_Usage, I2C1, i2cSensors)
#pragma config(Sensor, in1,    Gyro,           sensorGyro)
#pragma config(Sensor, in2,    liftPot,        sensorPotentiometer)
#pragma config(Sensor, I2C_1,  ,               sensorQuadEncoderOnI2CPort,    , AutoAssign )
#pragma config(Sensor, I2C_2,  ,               sensorQuadEncoderOnI2CPort,    , AutoAssign )
#pragma config(Motor,  port1,           leftBotLift,   tmotorVex393_HBridge, openLoop, reversed)
#pragma config(Motor,  port2,           rightMidLift,  tmotorVex393_MC29, openLoop, reversed)
#pragma config(Motor,  port3,           rightTopLift,  tmotorVex393_MC29, openLoop)
#pragma config(Motor,  port4,           leftMidLift,   tmotorVex393_MC29, openLoop)
#pragma config(Motor,  port5,           leftTopLift,   tmotorVex393_MC29, openLoop, reversed)
#pragma config(Motor,  port6,           rightDrive,    tmotorVex393HighSpeed_MC29, openLoop, reversed, driveRight, encoderPort, I2C_2)
#pragma config(Motor,  port7,           leftDrive,     tmotorVex393HighSpeed_MC29, openLoop, driveLeft, encoderPort, I2C_1)
#pragma config(Motor,  port8,           rightClaw,     tmotorVex393HighSpeed_MC29, openLoop, reversed)
#pragma config(Motor,  port9,           leftClaw,      tmotorVex393HighSpeed_MC29, openLoop)
#pragma config(Motor,  port10,          rightBotLift,  tmotorVex393_HBridge, openLoop)
//*!!Code automatically generated by 'ROBOTC' configuration wizard               !!*//

#pragma platform(VEX2)

#pragma competitionControl(Competition)

#include "Vex_Competition_Includes.c"

#include "drive.h"
#include "manualLift.h"
#include "auton.h"

//////////////////////////////////////////////////

const short leftbutton= 1;
const short centerbutton= 2;
const short rightbutton= 4;

int count = 0;

void waitforpress()
{
	while(nLCDButtons == 0){}
	wait1Msec (5);
}

void waitforrelease()
{
	while(nLCDButtons != 0){}
	wait1Msec (5);
}

//////////////////////////////////////////////////

void pre_auton()
{
	bStopTasksBetweenModes = true;

	nMotorEncoder(rightDrive) = 0;
	nMotorEncoder(leftDrive) = 0;
	SensorValue(Gyro) = 0;

	bLCDBacklight= true;

	clearLCDLine(0);
	clearLCDLine(1);

	//allows robot to restart from vexnet disconnect without reselecting autonomous code
	while( (nLCDButtons != kButtonCenter)  && bIfiRobotDisabled )
	{
		switch(count){
		case 0:
			displayLCDCenteredString(0, "No Autonomous");
			displayLCDCenteredString(1, "<		 Enter 		>");
			waitforpress();

			if (nLCDButtons == centerbutton)
			{
				waitforrelease();
				displayLCDCenteredString(0, "No Auton?");
				displayLCDCenteredString(1, "||YES||");
				wait1Msec (1000);
				waitforpress();
			}
			if(nLCDButtons == leftbutton)
			{
				waitforrelease();
				count = 3;
			}
			else if(nLCDButtons == rightbutton)
			{
				waitforrelease();
				count++;
			}
			break;

			/////////////////////////////////////////////////////////////////////////////////

		case 1:
			displayLCDCenteredString(0, "Fence Auton");
			displayLCDCenteredString(1, "<		 Enter 		>");
			waitforpress();

			if (nLCDButtons == centerbutton)
			{
				waitforrelease();
				displayLCDCenteredString(0, "Fence Auton?");
				displayLCDCenteredString(1, "||YES||");
				wait1Msec (1000);
				waitforpress();
			}
			if(nLCDButtons == leftbutton)
			{
				waitforrelease();
				count--;
			}
			else if(nLCDButtons == rightbutton)
			{
				waitforrelease();
				count++;
			}
			break;

			/////////////////////////////////////////////////////////////////////////////////

		case 2:
			displayLCDCenteredString(0, "Stars --> Cube");
			displayLCDCenteredString(1, "<		 Enter 		>");
			waitforpress();

			if (nLCDButtons == centerbutton)
			{
				waitforrelease();
				displayLCDCenteredString(0, "Stars --> Cube?");
				displayLCDCenteredString(1, "||YES||");
				wait1Msec (1000);
				waitforpress();
			}
			if(nLCDButtons == leftbutton)
			{
				waitforrelease();
				count--;
			}
			else if(nLCDButtons == rightbutton)
			{
				waitforrelease();
				count++;
			}
			break;

			/////////////////////////////////////////////////////////////////////////////////

		case 3:
			displayLCDCenteredString(0, "Cube --> Stars");
			displayLCDCenteredString(1, "<		 Enter 		>");
			waitforpress();

			if (nLCDButtons == centerbutton)
			{
				waitforrelease();
				displayLCDCenteredString(0, "Cube --> Stars?");
				displayLCDCenteredString(1, "||YES||");
				wait1Msec (1000);
				waitforpress();
			}
			if(nLCDButtons == leftbutton)
			{
				waitforrelease();
				count--;
			}
			else if(nLCDButtons == rightbutton)
			{
				waitforrelease();
				count = 0;
			}
			break;

			/////////////////////////////////////////////////////////////////////////////////

		default:
			displayLCDCenteredString(0, "No valid choice");
			displayLCDCenteredString(1, "was made!");
			break;

		}
	}
}

//////////////////////////////////////////////////

task autonomous()
{
	{
		bLCDBacklight= true;
		clearLCDLine(0);
		clearLCDLine(1);

		switch(count){

		case 0:
			displayLCDCenteredString(0, "No autonomous");
			displayLCDCenteredString(1, "is running!");
			wait1Msec(100);
			break;

			//////////////////////////////////////////////////////////////////////////////////////

		case 1:
			displayLCDCenteredString(0, "Fence Auton");
			displayLCDCenteredString(1, "is running!");
			wait1Msec(100);

			closeClaw(1000);
			wait(200);
			liftUp(1500);
			wait(200);
			forward(127, 500);
			wait(200);
			backward(127, 100);
			wait(200);
			leftTurn(127, 20);
			wait(200);
			forward(127, 100);
			wait(200);
			backward(127, 100);
			wait(200);
			rightTurn(127, 40);
			wait(200);
			forward(127,100);
			wait(2000);


			break;

			///////////////////////////////////////////////////////////////////////////////////////

		case 2:
			displayLCDCenteredString(0, "Stars --> Cube");
			displayLCDCenteredString(1, "is running!");
			wait1Msec(100);

			closeClaw(1000);
			wait(200);
			forward(127, 500);
			wait(200);
			openClaw(750);
			wait(200);
			liftUp(1250);
			wait(200);
			rightTurn(80, 45);
			wait(200);
			backward(127, 500);
			wait(200);
			rightTurn(80,45);
			wait(200);
			backward(127, 200);
			liftUpPot(1900);
			wait(2000);



			break;

			/////////////////////////////////////////////////////////////////////////////////////////////

		case 3:
			displayLCDCenteredString(0, "Cube --> Stars");
			displayLCDCenteredString(1, "is running!");
			wait1Msec(100);

			forward(127, 400);
			closeClaw(1000);
			wait(200);
			liftUp(1500);
			wait(200);
			rightTurn(127, 135);
			wait(200);
			backward(127, 500);
			liftUpPot(1900);
			wait(2000);


		}
	}
}

//////////////////////////////////////////////////

task usercontrol()
{
	while (true)
	{
		displayLCDCenteredString(0, "M + D");
		displayLCDCenteredString(1, "<3");

		if(getTaskState(driveTask) == taskStateStopped)
			startTask(driveTask);
		if(getTaskState(manualLift) == taskStateStopped)
			startTask(manualLift);
		if (getTaskState(claw) == taskStateStopped)
			startTask(claw);
	}
}
