#pragma config(UART_Usage, UART1, uartVEXLCD, baudRate19200, IOPins, None, None)
#pragma config(UART_Usage, UART2, uartNotUsed, baudRate4800, IOPins, None, None)
#pragma config(Sensor, in1,    Gyro,           sensorGyro)
#pragma config(Sensor, in2,    liftPot,        sensorPotentiometer)
#pragma config(Sensor, in3,    clawPot,        sensorPotentiometer)
#pragma config(Sensor, in4,    vertGyro,       sensorGyro)
#pragma config(Motor,  port1,           leftMidLift,   tmotorVex393_HBridge, openLoop, reversed)
#pragma config(Motor,  port2,           leftLift,      tmotorVex393_MC29, openLoop)
#pragma config(Motor,  port3,           rightLift,     tmotorVex393_MC29, openLoop, reversed)
#pragma config(Motor,  port4,           rightClaw,     tmotorVex393HighSpeed_MC29, openLoop, reversed)
#pragma config(Motor,  port5,           leftClaw,      tmotorVex393HighSpeed_MC29, openLoop)
#pragma config(Motor,  port6,           leftFront,     tmotorVex393TurboSpeed_MC29, openLoop)
#pragma config(Motor,  port7,           leftBack,      tmotorVex393TurboSpeed_MC29, openLoop)
#pragma config(Motor,  port8,           rightFront,    tmotorVex393TurboSpeed_MC29, openLoop, reversed)
#pragma config(Motor,  port9,           rightBack,     tmotorVex393TurboSpeed_MC29, openLoop, reversed)
#pragma config(Motor,  port10,          rightMidLift,  tmotorVex393_HBridge, openLoop, reversed)
//*!!Code automatically generated by 'ROBOTC' configuration wizard               !!*//

#pragma platform(VEX2)

#pragma competitionControl(Competition)

#include "Vex_Competition_Includes.c"

#include "drive.h"
#include "auton.h"
#include "manualLift.h"
#include "tasks.h"

//////////////////////////////////////////////////

const short leftbutton= 1;
const short centerbutton= 2;
const short rightbutton= 4;

int count = 0;

void waitforpress()
{
	while(nLCDButtons == 0){}
	wait1Msec (5);
}

void waitforrelease()
{
	while(nLCDButtons != 0){}
	wait1Msec (5);
}

//////////////////////////////////////////////////

void pre_auton()
{
	bStopTasksBetweenModes = true;

	nMotorEncoder(rightFront) = 0;
	nMotorEncoder(leftFront) = 0;
	SensorValue(Gyro) = 0;
	SensorValue(vertGyro) = 0;

	bLCDBacklight= true;

	clearLCDLine(0);
	clearLCDLine(1);

	//allows robot to restart from vexnet disconnect without reselecting autonomous code
	while( (nLCDButtons != kButtonCenter)  && bIfiRobotDisabled )
	{
		switch(count){
		case 0:
			displayLCDCenteredString(0, "No Autonomous");
			displayLCDCenteredString(1, "<		 Enter 		>");
			waitforpress();

			if (nLCDButtons == centerbutton)
			{
				waitforrelease();
				displayLCDCenteredString(0, "No Auton?");
				displayLCDCenteredString(1, "||YES||");
				wait1Msec (1000);
				waitforpress();
			}
			if(nLCDButtons == leftbutton)
			{
				waitforrelease();
				count = 6;
			}
			else if(nLCDButtons == rightbutton)
			{
				waitforrelease();
				count++;
			}
			break;

			/////////////////////////////////////////////////////////////////////////////////

		case 1:
			displayLCDCenteredString(0, "Fence Auton");
			displayLCDCenteredString(1, "<		 Enter 		>");
			waitforpress();

			if (nLCDButtons == centerbutton)
			{
				waitforrelease();
				displayLCDCenteredString(0, "Fence Auton?");
				displayLCDCenteredString(1, "||YES||");
				wait1Msec (1000);
				waitforpress();
			}
			if(nLCDButtons == leftbutton)
			{
				waitforrelease();
				count--;
			}
			else if(nLCDButtons == rightbutton)
			{
				waitforrelease();
				count++;
			}
			break;

			/////////////////////////////////////////////////////////////////////////////////

		case 2:
			displayLCDCenteredString(0, "C-->S LEFT");
			displayLCDCenteredString(1, "<		 Enter 		>");
			waitforpress();

			if (nLCDButtons == centerbutton)
			{
				waitforrelease();
				displayLCDCenteredString(0, "C-->S LEFT?");
				displayLCDCenteredString(1, "||YES||");
				wait1Msec (1000);
				waitforpress();
			}
			if(nLCDButtons == leftbutton)
			{
				waitforrelease();
				count--;
			}
			else if(nLCDButtons == rightbutton)
			{
				waitforrelease();
				count++;
			}
			break;

			/////////////////////////////////////////////////////////////////////////////////

		case 3:
			displayLCDCenteredString(0, "C-->S RIGHT");
			displayLCDCenteredString(1, "<		 Enter 		>");
			waitforpress();

			if (nLCDButtons == centerbutton)
			{
				waitforrelease();
				displayLCDCenteredString(0, "C-->S RIGHT?");
				displayLCDCenteredString(1, "||YES||");
				wait1Msec (1000);
				waitforpress();
			}
			if(nLCDButtons == leftbutton)
			{
				waitforrelease();
				count--;
			}
			else if(nLCDButtons == rightbutton)
			{
				waitforrelease();
				count++;
			}
			break;

			/////////////////////////////////////////////////////////////////////////////////

		case 4:
			displayLCDCenteredString(0, "C-->Bl LEFT");
			displayLCDCenteredString(1, "<		 Enter 		>");
			waitforpress();

			if (nLCDButtons == centerbutton)
			{
				waitforrelease();
				displayLCDCenteredString(0, "C-->Bl LEFT?");
				displayLCDCenteredString(1, "||YES||");
				wait1Msec (1000);
				waitforpress();
			}
			if(nLCDButtons == leftbutton)
			{
				waitforrelease();
				count--;
			}
			else if(nLCDButtons == rightbutton)
			{
				waitforrelease();
				count++;
			}
			break;

			/////////////////////////////////////////////////////////////////////////////////

		case 5:
			displayLCDCenteredString(0, "C-->Bl RIGHT");
			displayLCDCenteredString(1, "<		 Enter 		>");
			waitforpress();

			if (nLCDButtons == centerbutton)
			{
				waitforrelease();
				displayLCDCenteredString(0, "C-->Bl RIGHT?");
				displayLCDCenteredString(1, "||YES||");
				wait1Msec (1000);
				waitforpress();
			}
			if(nLCDButtons == leftbutton)
			{
				waitforrelease();
				count--;
			}
			else if(nLCDButtons == rightbutton)
			{
				waitforrelease();
				count++;
			}
			break;

			/////////////////////////////////////////////////////////////////////////////////

		case 6:
			displayLCDCenteredString(0, "Prog Skills");
			displayLCDCenteredString(1, "<		 Enter 		>");
			waitforpress();

			if (nLCDButtons == centerbutton)
			{
				waitforrelease();
				displayLCDCenteredString(0, "Prog Skills?");
				displayLCDCenteredString(1, "||YES||");
				wait1Msec (1000);
				waitforpress();
			}
			if(nLCDButtons == leftbutton)
			{
				waitforrelease();
				count--;
			}
			else if(nLCDButtons == rightbutton)
			{
				waitforrelease();
				count = 0;
			}
			break;

			/////////////////////////////////////////////////////////////////////////////////

		default:
			displayLCDCenteredString(0, "No valid choice");
			displayLCDCenteredString(1, "was made!");
			break;

		}
	}
}

//////////////////////////////////////////////////

task autonomous()
{
	{
		bLCDBacklight= true;
		clearLCDLine(0);
		clearLCDLine(1);

		nMotorEncoder(rightFront) = 0;
		nMotorEncoder(leftFront) = 0;
		SensorValue(Gyro) = 0;
		SensorValue(vertGyro) = 0;

		switch(count){

		case 0:
			displayLCDCenteredString(0, "No autonomous");
			displayLCDCenteredString(1, "is running!");
			wait1Msec(100);
			break;

			//////////////////////////////////////////////////////////////////////////////////////

		case 1:
			displayLCDCenteredString(0, "Fence Auton");
			displayLCDCenteredString(1, "is running!");
			wait1Msec(100);

			clawFunction(1750);
			liftUp(1750);
			forward(127, 1650);

			break;

			///////////////////////////////////////////////////////////////////////////////////////

		case 2:
			displayLCDCenteredString(0, "C-->S LEFT");
			displayLCDCenteredString(1, "is running!");
			wait1Msec(100);

			if(getTaskState(CSLeftDrive) == taskStateStopped)
				startTask(CSLeftDrive);
			if(getTaskState(CSLeftClaw) == taskStateStopped)
				startTask(CSLeftClaw);
			if(getTaskState(CSLeftLift) == taskStateStopped)
				startTask(CSLeftLift);


			//forward(127, 420);
			//wait(100);
			//leftTurn(-127, 30);
			//wait(100);
			//forward(127, 600);
			//wait(100);
			//clawFunction(800);
			//liftUp(1500);
			//forward(127, 200);
			//leftTurn(-127, 100);
			//clawFunction(400);
			//backward(127, 550);
			//liftUp(2200);
			//clawFunction(900);
			//liftDown(550);
			//forward(127, 700);
			//clawFunction(400);
			//forward(127, 50);
			//backward(127, 800);
			//liftUp(2200);
			//clawFunction(1000);
			//liftDown(550);
			//wait(2000);




			break;

			/////////////////////////////////////////////////////////////////////////////////////////////

		case 3:
			displayLCDCenteredString(0, "C-->S RIGHT");
			displayLCDCenteredString(1, "is running!");
			wait1Msec(100);


			if(getTaskState(CSRightDrive) == taskStateStopped)
				startTask(CSRightDrive);
			if(getTaskState(CSRightClaw) == taskStateStopped)
				startTask(CSRightClaw);
			if(getTaskState(CSRightLift) == taskStateStopped)
				startTask(CSRightLift);

			//forward(127, 420);
			//wait(100);
			//leftTurn(127, 30);
			//wait(100);
			//forward(127, 600);
			//wait(100);
			//clawFunction(800);
			//liftUp(1500);
			//forward(127, 200);
			//leftTurn(127, 100);
			//clawFunction(400);
			//backward(127, 550);
			//liftUp(2200);
			//clawFunction(900);
			//liftDown(550);
			//forward(127, 700);
			//clawFunction(400);
			//forward(127, 50);
			//backward(127, 800);
			//liftUp(2200);
			//clawFunction(1000);
			//liftDown(550);
			//wait(2000);

			break;

			//////////////////////////////////////////////////////////////////////////////////////

		case 4:
			displayLCDCenteredString(0, "C-->Bl LEFT");
			displayLCDCenteredString(1, "is running!");
			wait1Msec(100);

			if(getTaskState(CSLeftBlDrive) == taskStateStopped)
				startTask(CSLeftBlDrive);
			if(getTaskState(CSLeftBlClaw) == taskStateStopped)
				startTask(CSLeftBlClaw);
			if(getTaskState(CSLeftBlLift) == taskStateStopped)
				startTask(CSLeftBlLift);

			break;

			//////////////////////////////////////////////////////////////////////////////////////


		case 5:
			displayLCDCenteredString(0, "C-->Bl RIGHT");
			displayLCDCenteredString(1, "is running!");
			wait1Msec(100);

			if(getTaskState(CSRightBlDrive) == taskStateStopped)
				startTask(CSRightBlDrive);
			if(getTaskState(CSLeftBlClaw) == taskStateStopped) // CSRightBlClaw & CSLeftBLClaw sub for less tasks
				startTask(CSLeftBlClaw);
			if(getTaskState(CSLeftBlLift) == taskStateStopped)
				startTask(CSLeftBlLift);

			break;

			//////////////////////////////////////////////////////////////////////////////////////

		case 6:
			displayLCDCenteredString(0, "Prog Skills");
			displayLCDCenteredString(1, "is running!");
			wait1Msec(100);

			if(getTaskState(PSDrive) == taskStateStopped)
				startTask(PSDrive);
			if(getTaskState(PSClaw) == taskStateStopped)
				startTask(PSClaw);
			if(getTaskState(PSLift) == taskStateStopped)
				startTask(PSLift);


			//////////////////////////////////////////////////////////////////////////////////////

		}
	}
}

//////////////////////////////////////////////////

task usercontrol()
{
	SensorValue(vertGyro) = 0;

	while (true)
	{
		displayLCDCenteredString(0, "M + D");
		displayLCDCenteredString(1, "<3");

		if(getTaskState(driveTask) == taskStateStopped)
			startTask(driveTask);
		if(getTaskState(manualLift) == taskStateStopped)
			startTask(manualLift);
		if (getTaskState(claw) == taskStateStopped)
			startTask(claw);
	}
}
